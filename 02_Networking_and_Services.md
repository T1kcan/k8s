...
- Establish connection to the Kubernetes Cluster and check whether running and nodes are ready.
...
- Create a `curl-test.yaml` file to create a Pod that pings a Pod inside the cluster.
...
- Create a `curl-test` pod and log into the container.
...
- Create a `curl-test.yaml` file to create a Pod that connects the web-flask page via curl command.
...
- Create the `curl-test` pod and log into the container. Connects the web-flask page via curl command and pay attention to load balancig.
